...
use crate::ffi::*;
use crate::module::loader::{ExtModuleLoader, ModuleLoader};
...
impl Runtime {
  pub fn new(options: RuntimeOptions) -> Box<Self> {
    ...
    let loader = options
      .loader
      .unwrap_or_else(|| Rc::new(ExtModuleLoader::new()));
  ...
  fn init_module_loader(&self) {
    unsafe {
      JS_SetModuleLoaderFunc(
        self.inner.as_ptr(),
        Some(Self::module_resolver_unsafe),
        Some(Self::module_loader_unsafe),
